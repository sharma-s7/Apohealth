...
const Navbar = () => {
  ...
  const logout = () => {
    setToken(false); // don't use string 'false'
    localStorage.removeItem('token')
    navigate('/login'); // redirect user after logout (optional)
  }
  ...
  return (
    <div className='flex items-center justify-between text-sm py-4 mb-4 border-b border-b-gray-400'>
     < img onClick={()=>navigate('/')} className="w-70 cursor-pointer transition-transform duration-300 hover:scale-105"  src={assets.ap_logo} alt="" />
     ...
     <div className='flex items-center gap-4'>
        {
            ...
            <img className='w-2.5'src={assets.dropdown_icon} alt="" />
                <div className='absolute top-0 right-0 pt-14 text-base font-medium text-black z-20 hidden group-hover:block'>
                    <div className='min-w-48 bg-blue-200 rounded flex flex-col gap-4 p-4'>
                        <p onClick={()=>navigate('my-profile')}className='hover:text-white cursor-pointer'>My Profile</p>
                        <p onClick={()=>navigate('my-appointments')}className='hover:text-white cursor-pointer'>MyAppointment</p>
